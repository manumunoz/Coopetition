...
class Intro(Page):
    ...
    def is_displayed(self):
        ...
        if not 'total_codes' in self.participant.vars.keys():
            self.participant.vars['total_codes'] = 0
        # end if
        if not 'good_codes' in self.participant.vars.keys():
            self.participant.vars['good_codes'] = 0
        # end if
        if not 'bad_codes' in self.participant.vars.keys():
            self.participant.vars['bad_codes'] = 0
        # end if
...
class Play(Page):
    ...
    timer_text = 'Time left in Part 1:'
    ...
    def before_next_page(self):
        ...
        self.participant.vars['total_codes'] += 1
        if (sol[0] == wc1) and (sol[1] == wc2) and (sol[2] == wc3):
            self.participant.vars['good_codes'] += 1
        else:
            self.participant.vars['bad_codes'] += 1
        # end if
        print('self.participant.vars.total_codes =',self.participant.vars['total_codes'])
        print('self.participant.vars.good_codes  =',self.participant.vars['good_codes'] )
        print('self.participant.vars.bad_codes   =',self.participant.vars['bad_codes']  )
    # end def
...
class Results(Page):
    ...
    def is_displayed(self):
        self.player.total_codes = self.participant.vars['total_codes']
        self.player.good_codes  = self.participant.vars['good_codes']
        self.player.bad_codes   = self.participant.vars['bad_codes']
